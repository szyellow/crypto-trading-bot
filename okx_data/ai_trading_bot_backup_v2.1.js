...
// 检查买入冷却期
function checkBuyCooldown(coin) {
    ...
    if (diffMinutes < AI_CONFIG.buyCooldownMinutes) {
        const remaining = Math.ceil(AI_CONFIG.buyCooldownMinutes - diffMinutes);
        return { canBuy: false, remainingMinutes: remaining };
    }
    
    return { canBuy: true, remainingMinutes: 0 };
}
...
async function makeDecision(coin, position, currentPrice, sentiment, account) {
    ...
    if (pos && pos.amount > 0 && pos.avgPrice > 0) {
        ...
        if (pnlPercent <= smartStopLoss) {
            // 如果趋势评分很高，考虑补仓而不是止损
            if (sentiment.score >= 8 && positionPercent < 15 && usdtAvailable >= AI_CONFIG.tradeAmount) {
                // 补仓降低成本
                const buyAmount = Math.min(AI_CONFIG.tradeAmount, usdtAvailable * 0.5);
                decision = { 
                    action: 'buy', 
                    reason: `智能补仓！亏损${pnlPercent.toFixed(2)}%但趋势评分${sentiment.score}/10强劲，补仓降低成本`,
                    amount: buyAmount
                };
                return decision;
            }
    ...
    // 检查连续加仓限制：已有持仓且占比>1%的币种，禁止继续买入（极严格限制）
    if (positionPercent > 1) {
        decision = { action: 'hold', reason: `${coin}已有持仓且占比${positionPercent.toFixed(1)}%>1%，禁止连续加仓（确保高抛后再低吸）` };
        return decision;
    }
    ...
    if (sentiment.score >= AI_CONFIG.sentimentThreshold) {
        decision = { 
            action: 'buy', 
            reason: `舆情利好(${sentiment.score}分)，买入${AI_CONFIG.tradeSize} USDT`,
            amount: AI_CONFIG.tradeSize / currentPrice
        };
        return decision;
    }
...
async function executeTrade(instId, side, amount, price, maxRetries = 5, usdtAmount = null) {
    ...
    const orderUsdtAmount = isBuy && usdtAmount ? usdtAmount : (amount * price);
    
    // 验证买入金额不超过限制
    if (isBuy && orderUsdtAmount > AI_CONFIG.tradeSize * 1.01) {
        console.error(`❌ 买入金额超标！计划${orderUsdtAmount} USDT，限制${AI_CONFIG.tradeSize} USDT`);
        // 重新计算数量
        amount = AI_CONFIG.tradeSize / price;
        console.log(`🔄 已调整买入数量: ${amount.toFixed(6)}`);
    }
        ...
        try {
            ...
            if (side === 'buy') {
                ...
                orderBody = {
                    ...
                    side: side,
                    ordType: 'market',
                    sz: orderUsdtAmount.toFixed(2)  // 使用USDT金额，保留2位小数
                };
                console.log(`🎯 使用市价单买入 ${orderUsdtAmount.toFixed(2)} USDT`);
            } else {
                // 卖出：使用市价单，sz表示币种数量
                orderType = 'market';
                orderBody = {
...
async function aiTrading() {
    console.log('=== AI自主交易系统 v2.1 - 智能发现模式 ===');
    ...
    for (const coin of uniqueCoins) {
        ...
        const cooldown = checkBuyCooldown(coin.symbol);
        ...
        if (!isBlacklisted && 
            positionPercent <= 1 &&  // 极严格限制：只有持仓<=1%才允许买入（几乎无持仓）
            ...
            console.log(`  💡 发现机会！${coin.symbol} 趋势评分 ${coin.potential}/10，现金充足${cashPercent.toFixed(1)}%，无持仓，冷却期OK`);
            decision.action = 'buy';
            ...
            } else if (positionPercent > 1) {
                console.log(`  🚫 ${coin.symbol}已有持仓${positionPercent.toFixed(1)}%>1%，禁止买入`);
            } else if (justBought) {
        ...
        if (decision.action !== 'hold') {
            ...
            if (result && result.code === '0') {
                ...
                if (decision.action === 'buy') {
                    ...
                    if (tpResult.success) {
                        console.log(`✅ ${coin.symbol} 止盈单已挂，目标 +${tpPercent.toFixed(2)}%`);
                    } else {
    ...
    // 自动复盘并迭代 - 增强版
    await autoReviewAndEvolve(tradeLog, positions, totalEquity);
}
...
async function autoReviewAndEvolve(tradeLog, positions, totalEquity) {
    ...
    if (winRate < 30 && sellTrades.length >= 3) {
        const newStopLoss = AI_CONFIG.stopLossPercent * 0.8; // 收紧20%
        adjustments.push(`胜率${winRate.toFixed(1)}%过低，止损从${AI_CONFIG.stopLossPercent}%收紧至${newStopLoss.toFixed(2)}%`);
    ...
    // 规则3: 平均持仓时间过长，优化选股
    if (avgHoldTime > 6) {
        const newThreshold = AI_CONFIG.sentimentThreshold + 1;
        adjustments.push(`平均持仓${avgHoldTime.toFixed(1)}小时过长，提高选股门槛至${newThreshold}分`);
        AI_CONFIG.sentimentThreshold = Math.min(10, newThreshold);
        notifications.push(`⚠️ 自动迭代：持仓时间过长(${avgHoldTime.toFixed(1)}小时)，已提高选股门槛至${newThreshold}分`);
    }
    ...
    if (cashPercent < 30) {
        const newMaxPositions = Math.max(2, AI_CONFIG.maxPositions - 1);
        adjustments.push(`现金比例${cashPercent.toFixed(1)}%过低，减少最大持仓至${newMaxPositions}个`);
        AI_CONFIG.maxPositions = newMaxPositions;
        notifications.push(`⚠️ 自动迭代：现金比例过低(${cashPercent.toFixed(1)}%)，已减少最大持仓至${newMaxPositions}个`);
    }
