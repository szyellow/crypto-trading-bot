...
// 导入币市麻雀战法配置 - v4.1 新增
// ============================================
const SPARROW_CONFIG = require('./config-sparrow.js');
...
function getCurrentTimeZone() {
    ...
    if (hour >= 0 && hour < 4) return '00:00-04:00';      // 亚洲尾盘
    if (hour >= 4 && hour < 8) return '04:00-08:00';      // 欧美交接
    if (hour >= 8 && hour < 12) return '08:00-12:00';     // 亚洲早盘
    if (hour >= 12 && hour < 16) return '12:00-16:00';    // 亚洲午盘
    if (hour >= 16 && hour < 20) return '16:00-20:00';    // 欧洲早盘
    return '20:00-24:00';                                  // 美国早盘
}

function getTimeZoneConfig() {
    const timeZone = getCurrentTimeZone();
    return SPARROW_CONFIG.timeZones[timeZone];
}

function getCheckInterval() {
    const config = getTimeZoneConfig();
    return config.intensity >= 4 ? SPARROW_CONFIG.checkInterval.active : SPARROW_CONFIG.checkInterval.quiet;
}

function getPositionSizeByTimeZone() {
    const config = getTimeZoneConfig();
    return config.positionSize;
}

function printTimeZoneInfo() {
    const timeZone = getCurrentTimeZone();
    const config = getTimeZoneConfig();
    const intensity = '⭐'.repeat(config.intensity);
    
    console.log(`\n🐦 币市麻雀战法 v4.1 - 时区感知`);
    console.log(`⏰ 当前时段: ${timeZone}`);
    console.log(`📊 活跃强度: ${intensity}`);
    console.log(`💰 建议仓位: $${config.positionSize.min}-$${config.positionSize.max}`);
    console.log(`⏱️ 持仓时间: ${config.holdTime.min}-${config.holdTime.max}分钟`);
    console.log(`🎯 日目标占比: ${config.dailyQuota * 100}%`);
    console.log(`🔄 检查频率: ${getCheckInterval()}分钟\n`);
}
...
async function aiTrading() {
    console.log('=== AI自主交易系统 v4.1 - 币市麻雀战法（时区感知版）===');
...
// 输出策略优化配置
console.log('\n🔧 策略优化配置已生效(v4.1-币市麻雀战法):');
console.log(`  • 日目标: $${SPARROW_CONFIG.dailyTarget} (1%)`);
...
console.log(`  • 日度控制: 盈利$${SPARROW_CONFIG.dailyControl.profitTarget}或亏损$${SPARROW_CONFIG.dailyControl.lossLimit}停止`);
